 ...
 */
class EMysql implements DatabaseDriverInterface {
	...
	public static function load(){
		...
		self::$db_name = EConfig::$data["database"]["name"];
		self::$db_host = EConfig::$data["database"]["host"];
		self::$db_user = EConfig::$data["database"]["user"];
		self::$db_pass = EConfig::$data["database"]["password"];
	...
	{
		//opening session
		$db = mysqli_connect(self::$db_host, self::$db_user, self::$db_pass); //self::$status = 2;
		/* check connection */
		if (!mysqli_connect_errno()) {
			$db_select = mysqli_select_db($db, self::$db_name); //self::$status = 1;
	...
	public static function safe($s){
		if(is_array($s)){
			foreach($s as $key => $value){
				$s[$key] = mysqli_real_escape_string(self::$db_link, $s[$key]);
			}
			return $s;
		} else {
			$s = mysqli_real_escape_string(self::$db_link, $s);
			return $s;
		}
	}
	...
	public static function q($q){
		if(self::$opened==true){
			...
			$ret = mysqli_query(self::$db_link, $q);
	...
	public static function table_exists($table){
		$r = self::q("SHOW TABLES LIKE '$table'");
		if(self::$opened) {
