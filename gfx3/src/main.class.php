...
class ELoader{
	...
	//include just a class name selectively
	public static function include_source($filename)
	{
...
if (EConfig::$data['generic']['database'] == "yes"){
	EModel::set_driver(EConfig::$data['generic']['databasedriver']);
	$driver = EModel::get_driver();
	
	//database driver inclusion
	ELoader::include_source("drivers/$driver");
	
	$interface = class_implements($driver);
	if(key($interface)=="DatabaseDriverInterface"){
	$driver::load();
	//EModel::get_driver()::load(); //uncomment on no backwards-compatible versions of gfx
	$unloader = new EUnloader();
	} else {
		ELog::warning("Unsupported class used as database driver! Change generic.conf.php databasedriver to an available driver.");
		$classes = get_declared_classes();
		foreach($classes as $class){
			$interface = class_implements($class);
			if(key($interface)=="DatabaseDriverInterface"){
				$provider = $class::provider();
				echo "Available driver: ".$class." => ".$provider."<br>";
			}
		}
	}
}
